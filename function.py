...
def PSNR_VAL(im_true, im_fake):
    ...
    Ifake = im_fake.clone()
    mse = nn.MSELoss(reduce=False)
    err = torch.sum(mse(Itrue, Ifake), dim=(1,2,3)) / (C*H*W)
    psnr = 10. * np.log((data_range**2)/(err.data + esp)) / np.log(10.)
    psnr_avg = psnr.mean()
    return psnr_avg
...
def RMSE_GPU(im_true, im_fake):
    ...
    aux = torch.sum(err, dim=(1, 2)) / (H*W)
    rmse_per_band = torch.sqrt(aux + esp)
    mse = torch.sum(err, dim=(0, 1, 2))/ (C*H*W)
    rmse = torch.sqrt(mse + esp)

    return rmse_per_band, rmse


def ERGAS_GPU(im_true, im_fake, ratio):
    C = im_true.size()[0]
    H = im_true.size()[1]
    W = im_true.size()[2]
    Itrue = im_true.clone().clamp(0, 1)
    ...
    mean_true = torch.sum(Itrue, dim=(1, 2))/(H*W)
    rmse_per_band, _ = RMSE_GPU(Itrue, Ifake)
    ergas = 100*ratio*torch.sqrt(torch.sum((rmse_per_band / mean_true)**2)/C + esp)
